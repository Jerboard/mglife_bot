...
def get_edit_flagman_kb(all_flagman: tuple[db.FlagmanRow]) -> InlineKeyboardMarkup:
    ...
    kb.button(text='✏️ Изменить название пакета', callback_data=f'edit_flagman_update_pack:name:{pack_id}')
    kb.button(text='➕ Добавить канал', callback_data=f'edit_flagman_update_pack:add_channel:{pack_id}')
    for flagman in all_flagman:
...
def get_edit_channel_kb(flagman: db.FlagmanRow) -> InlineKeyboardMarkup:
    ...
    kb.button(text='✏️ Изменить название кнопки',
              callback_data=f'edit_flagman_update_channel:button:{flagman.id}')
    kb.button(text='✏️ Обновить канал',
              callback_data=f'edit_flagman_update_channel:channel:{flagman.id}')
